...
项目特性
Gorm.Model集成: 所有模型都使用gorm.Model，自动包含ID、CreatedAt、UpdatedAt和DeletedAt字段
软删除支持: 通过gorm.Model的DeletedAt字段实现软删除
自动时间戳: 自动管理创建和更新时间
...
DeletedAt: 软删除时自动设置
环境配置
复制config.env.example为config.env并配置：
...
assessments - 评估表
answers - 答案表
face_detections - 人脸检测表
开发说明
添加新模型
在internal/models/目录创建新模型文件
使用gorm.Model作为基础结构
在inits/database.go中添加模型到AutoMigrate
创建对应的控制器和服务
数据库操作示例
// 查询
